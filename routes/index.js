...
router.route('/')
    ...
    .post(async (req, res) => {
        const hashtag = await Hashtag.findOne({ where : { name : req.body.searchKey } });
...
router.get('/profile', isLoggedIn,async (req, res) => {
    ...
    for(let i = 0; i < res.locals.user.Sales.length; i++) {
        ...
        res.locals.user.SalesFull += res.locals.user.Sales[i].saleCount * res.locals.user.Sales[i].price;
    }
    ...
    for(let i = 0; i < res.locals.user.Purchases.length; i++) {
        res.locals.user.Purchases[i].User = await res.locals.user.Purchases[i].getUser();
        res.locals.user.Purchases[i].Sale = await res.locals.user.Purchases[i].getSale();
    ...
    for(let i = 0; i < res.locals.user.Comments.length; i++) {
        res.locals.user.Comments[i].Sale = await res.locals.user.Comments[i].getSale();
    }
    res.locals.user.SalesComments = [];
    for(let i = 0; i < res.locals.user.Sales.length; i++) {
        res.locals.user.SalesComments = res.locals.user.SalesComments.concat(await res.locals.user.Sales[i].getComments());
    }
    res.locals.user.score = res.locals.user.SalesComments.length > 0 ? res.locals.user.SalesComments.map(e => +e.score).reduce((acc, cur) => acc + cur) / res.locals.user.SalesComments.length * 100 : 0;
...
router.post('/imgs/add', (req, res) => {
    fs.writeFileSync(path.join(__dirname, '../', req.body.src), req.body.img, 'base64');
});
